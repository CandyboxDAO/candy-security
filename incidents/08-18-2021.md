...
A low severity bug was discovered in the Candybox protocol on August 8, 2021. Projects that had received payments while configured with a reserved rate of 0%, who later reconfigured the reserved rate to anything greater than 0%, would have been affected. Luckily, [SharkDAO](https://candybox.money/#/p/sharkdao) was the only project affected. A temporary workaround has been identified, and a game plan for removing the bug from the system is in the works.
...
To give them time to plan for for what's next, SharkDAO needed a way to pause payments and new members coming in through Candybox. At the time of the incident, Candybox didn't support a pause function in its [TerminalV1](https://github.com/candyboxdao/candy-candyhouse/blob/main/packages/hardhat/contracts/TerminalV1.sol) contract, so Candybox devs had to improvise alongside the SharkDAO community.

First step was to remove the payment form from the UI. This keeps most people from sending in contributions, but someone dedicated to sending a payment would eventually find that the Candybox contract could still be interacted with directly through other interfaces, like Etherscan.
...
* SharkDAO's Candybox project is owned by a [Gnosis](https://gnosis.io/) multi-sig requiring 3/5 signatures. I'm one of the signatories.
...
Despite having written extensive unit, integration, and load tests, this condition fell through the cracks. It is important to be forthright about the fact that Candybox is still experimental software with high risks. We're confident in the way it is built and are committed to its improvement, but it is still in its infancy.

TerminalV1 has a built in mechanism to allow projects to choose to migrate to new terminal contracts that have been approved by CandyboxDAO's governance, so bugs like this can be fixed by making future Terminal versions with improvements.
...
* Consider building a version of the Candybox Terminal contract that supports pausing payments.
